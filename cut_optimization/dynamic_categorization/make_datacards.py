...
def add_data(w, cat_name, input_path, data_tree, cut, method):
    ...
    Import(w, data)
    return w.data("%s_data"%cat_name), bkg_entries
...
def make_dnn_categories(categories, sig_input_path, data_input_path, data_tree, output_path, filename, statUnc=False, nuis=False, res_unc_val=0.1, scale_unc_val=0.0005, smodel='3gaus', method="", lumi=40000):
    ...
    for cat_name, cut in categories.iteritems():
        ...
        if '3gaus' in smodel:
            ...
            if nuis:
                sig_rate, sig_entries = add_sig_model_with_nuisances(w, cat_name, sig_input_path, cut, res_unc_val, scale_unc_val, method, lumi)    
            else:
                sig_rate, sig_entries = add_sig_model(w, cat_name, sig_input_path, cut, method, lumi) 
        elif 'dcb' in smodel:
            if nuis:
                sig_rate, sig_entries = add_sig_model_dcb_with_nuisances(w, cat_name, sig_input_path, cut, res_unc_val, scale_unc_val, method, lumi)    
            else:
