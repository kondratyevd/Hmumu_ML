...
cd $TMP_PATH
python $CURRENT_LOCATION/categorize_dynamic.py --option $1 --smodel 'dcb' --sig_in_path "$SIG_INPUT_PATH" --data_in_path "$DATA_INPUT_PATH" --out_path "$TMP_PATH"  --data_tree "$DATA_TREE" --method $method --min_var $min_var --max_var $max_var --nSteps $nSteps --lumi $LUMI --penalty $penalty
