...
LUMI=35866
OUTPUT_PATH=output/ggh_2017_psweights_nuis_0_dcb/
mkdir $OUTPUT_PATH
python categorize.py --nuis --nuis_val 0 --smodel dcb --sig_in_path "$SIG_INPUT_PATH" --data_in_path "$DATA_INPUT_PATH" --out_path "$OUTPUT_PATH" --sig_tree "$SIG_TREE" --data_tree "$DATA_TREE" --lumi "$LUMI"
cd $OUTPUT_PATH
