...
def add_data(w, cat_number, input_path, data_tree, cut):
    ...
    dummy.Close()
    data = ROOT.RooDataSet("cat%i_data"%cat_number,"cat%i_data"%cat_number, data_tree, ROOT.RooArgSet(var, max_abs_eta_var, mu1_eta, mu2_eta, ggh_pred_var, vbf_pred_var, dy_pred_var, tt_pred_var), cut)
    Import(w, data)
...
def add_sig_model(w, cat_number, input_path, cut):
    ...
    w.factory("cat%i_mix1 [0.5, 0.0, 1.0]"%cat_number)
    w.factory("cat%i_mix2 [0.5, 0.0, 1.0]"%cat_number)
    mix1 = w.var("cat%i_mix1"%cat_number)
    mix2 = w.var("cat%i_mix2"%cat_number)
...
def add_sig_model_with_nuisances(w, cat_number, input_path, cut, res_unc_val, scale_unc_val):
    var = w.var("mass")
    var.setBins(5000)
    max_abs_eta_var = ROOT.RooRealVar("max_abs_eta_mu","Max abs(eta) of muons", 0, 2.4) 
    mu1_eta = ROOT.RooRealVar("mu1_eta","mu1_eta", -2.4, 2.4) 
    ...
    w.factory("cat%i_mix1 [0.5, 0.0, 1.0]"%cat_number)
    w.factory("cat%i_mix2 [0., 0.0, 1.0]"%cat_number)
...
def add_sig_model_dcb(w, cat_number, input_path, cut):
    var = w.var("mass")
    var.setBins(5000)
    max_abs_eta_var = ROOT.RooRealVar("max_abs_eta_mu","Max abs(eta) of muons", 0, 2.4) 
    mu1_eta = ROOT.RooRealVar("mu1_eta","mu1_eta", -2.4, 2.4) 
...
def add_sig_model_dcb_with_nuisances(w, cat_number, input_path, cut, res_unc_val, scale_unc_val):
    var = w.var("mass")
    var.setBins(5000)
    max_abs_eta_var = ROOT.RooRealVar("max_abs_eta_mu","Max abs(eta) of muons", 0, 2.4) 
    mu1_eta = ROOT.RooRealVar("mu1_eta","mu1_eta", -2.4, 2.4) 
...
def make_dnn_categories(score, bins, sig_input_path, data_input_path, data_tree, output_path, filename, statUnc=False, nuis=False, res_unc_val=0.1, scale_unc_val=0.0005, smodel='3gaus'):
    nCat = len(bins)-1
    cat_names = []
    combine_import = ""
    combine_bins = "bin         "
    ...
    for i, b in enumerate(bins):
        ...
        cut = "((%s>%.5f)&(%s<%.5f))"%(score, bin_min, score, bin_max)
        print "Applying cut: ", cut
        if '3gaus' in smodel:
            if nuis:
                sig_rate = add_sig_model_with_nuisances(w, i, sig_input_path, cut, res_unc_val, scale_unc_val)    
            else:
                sig_rate = add_sig_model(w, i, sig_input_path, cut) 
        elif 'dcb' in smodel:
            if nuis:
                sig_rate = add_sig_model_dcb_with_nuisances(w, i, sig_input_path, cut, res_unc_val, scale_unc_val)    
            else:
                sig_rate = add_sig_model_dcb(w, i, sig_input_path, cut) 
...
def create_datacard_dnn(score, bins, sig_in_path, data_in_path, data_tree, out_path, name, workspace_filename, statUnc=False, nuis=False, res_unc_val=0.1, scale_unc_val=0.0005, smodel='3gaus'): 
    print "="*30
    ...
    try:
        ...
        if e.errno != errno.EEXIST:
            raise
    import_str, bins_obs, cat_strings, unc_str = make_dnn_categories(score, bins, sig_in_path, data_in_path, data_tree, out_path, workspace_filename, statUnc=statUnc, nuis=nuis, res_unc_val=res_unc_val, scale_unc_val=scale_unc_val, smodel=smodel)
