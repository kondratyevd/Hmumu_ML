...
def add_sig_model_3gaus(w, cat_number, input_path, sig_tree, lumi, cut):
    var = w.var("mass")
    var.setBins(5000)
    max_abs_eta_var = ROOT.RooRealVar("max_abs_eta_mu","Max abs(eta) of muons", 0, 2.4) 
    mu1_eta = ROOT.RooRealVar("mu1_eta","mu1_eta", -2.4, 2.4) 
...
def add_sig_model_3gaus_with_nuisances(w, cat_number, input_path, sig_tree, lumi, cut, nuis_val):
    var = w.var("mass")
    var.setBins(5000)
    max_abs_eta_var = ROOT.RooRealVar("max_abs_eta_mu","Max abs(eta) of muons", 0, 2.4) 
    mu1_eta = ROOT.RooRealVar("mu1_eta","mu1_eta", -2.4, 2.4) 
...
def make_eta_categories(bins, sig_input_path, sig_tree, data_input_path, data_tree, output_path, filename, lumi, nuis=False, nuis_val=0.1, smodel='3gaus'):
    nCat = len(bins)-1
    cat_names = []
    combine_import = ""
    combine_bins = "bin         "
    ...
    for i, b in enumerate(bins):
        ...
        if '3gaus' in smodel:
            if nuis:
                sig_rate = add_sig_model_3gaus_with_nuisances(w, i, sig_input_path, sig_tree, lumi, cut, nuis_val)    
            else:
                sig_rate = add_sig_model_3gaus(w, i, sig_input_path, sig_tree, lumi, cut) 
        elif 'dcb' in smodel:
            if nuis:
...
def create_datacard(bins, sig_in_path, sig_tree, data_in_path, data_tree, out_path, name, workspace_filename, lumi, nuis=False, nuis_val=0.1, smodel='3gaus'): 
    print "="*30
    ...
    try:
        ...
        if e.errno != errno.EEXIST:
            raise
    import_str, bins_obs, cat_strings= make_eta_categories(bins, sig_in_path, sig_tree, data_in_path, data_tree, out_path, workspace_filename, lumi, nuis=nuis, nuis_val=nuis_val, smodel=smodel)
...
create_datacard(bins, signal_input, "tree", data_input, "dimuons/tree", "./",  "datacard_3Gaus_nuis_new_false", "workspace_3Gaus_nuis_new_false", 35866, nuis=True, nuis_val=0.1, smodel='3gaus')
